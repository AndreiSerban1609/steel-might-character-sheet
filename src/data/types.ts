...
/**
 * Spell tags (demo feedback #21, taxonomy ruled 2026-08-13). A spell carries as many as
 * apply — a fireball is `damage` + `fire` + `aoe`. Role tags describe what the spell DOES;
 * the 11 element tags mirror the damage types so "show me every ice spell" is one filter.
 * Generated by `npm run tag-spells`, which is re-runnable and idempotent.
 */
export type SpellRoleTag =
  | 'damage' | 'healing' | 'buff' | 'debuff' | 'control' | 'utility'
  | 'summon' | 'movement' | 'defense' | 'ritual' | 'aoe';

export type SpellTag = SpellRoleTag | DamageType;
...
export interface Spell {
  ...
  /**
   * Save outcome (Game Owner ruling 2026-08-27): a successful save means NO damage and no
   * effects, unless the description says the target still takes half — then this is true.
   * Generated by `npm run flag-half-on-save` from the description text; absent = false.
   */
