...
// Spell tags (demo feedback #21): role tags + the 11 element tags, generated by
// scripts/tag-spells.ts. Validated here so a hand edit can't invent a tag the UI
// filter would never show.
const validSpellTags = new Set([
...
for (const spell of allSpells) {
  ...
  if (!Array.isArray(spell.tags) || spell.tags.length === 0) {
    error(`spell [${spell.id}]: missing tags — run \`npm run tag-spells\``);
  } else {
    for (const tag of spell.tags) {
      if (!validSpellTags.has(tag)) error(`spell [${spell.id}]: unknown tag "${tag}"`);
    }
  }
